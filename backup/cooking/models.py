...
class Showcooking(models.Model):
    ...
    id=models.AutoField(primary_key=True)
    imagen = models.ImageField(upload_to='imagenes/', null=False, blank=False, default='showcookings/default.jpg')
    ...
    categoria = models.ForeignKey(categoria_showcooking, on_delete=models.CACADE, null=False , blank=False)
    Dificultad_Niveles = [
        ('facil', 'Fácil'),
    ...
    def __str__(self):
        return self.titulo
    def get_cocineros(self):
        return ", ".join([cocinero.username for cocinero in self.cocineros.all()])
...
class Receta(models.Model):
    ...
    categoria = models.ForeignKey(Categoria_receta, on_delete=models.CASCADE)

#probar a ver como funciona el foreignkey
class Chef_ShowCooking(models.Model):
    id_showcooking=models.ForeignKey(Showcooking)
    id_chef=models.ForeignKey(settings.AUTH_USER_MODEL)
class Chef_Recetas(models.Model):
    id_chef=models.ForeignKey(settings.AUTH_USER_MODEL)
    id_receta=models.ForeignKey(Receta)
...
class ingredientes_receta(models.Model):
    id_Receta=models.ForeignKey(Receta)
    id_ingrediente=models.ForeignKey(ingredientes)
